...
class UserService:
    ...
    async def get_users(self, session: AsyncSession) -> list[GetUser]:
        async with session.begin():
            ...
            if users_data is None:
                return []
            return [GetUser.model_validate(user, from_attributes=True) for user in users_data]
    ...
    async def get_user_by_email(self, email: EmailStr, session: AsyncSession) -> GetUser:
        async with session.begin():
            user_repo = UserRepo(session)
    ...
    async def update_user(self, id: int, data_to_update: UpdateUser, session: AsyncSession) -> GetUser:
        async with session.begin():
            user_repo = UserRepo(session)
            data_to_update = data_to_update.model_dump()
            updated_data = await user_repo.update_user(id=id, 
